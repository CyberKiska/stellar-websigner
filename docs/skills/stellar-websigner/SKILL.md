...
| Plain Text | UTF-8 of textarea **DOM value**, no trimming or Unicode normalization. Browser newline normalization applies. Maximum **1,048,576 UTF-8 bytes (1 MiB)**, not characters. Whitespace is content. UI requires nonempty text, although core code supports empty text. File mode and text mode are distinct even for identical bytes. Autocorrect/autocapitalize/spellcheck are off so the signed text is what was typed. |
